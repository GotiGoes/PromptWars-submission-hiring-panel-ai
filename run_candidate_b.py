"""Run live evaluation pipeline for Candidate B (Ananya Iyer)."""
...
def main():
    print("=" * 80)
    print("      LIVE EVALUATION PIPELINE: CANDIDATE B (ANANYA IYER)")
    print("=" * 80)
    ...
    # 1. Load Input Data for Candidate B & Job Description
    base_dir = Path(__file__).parent / "sample_data"
    resume_path = base_dir / "candidate_b" / "resume.txt"
    transcript_path = base_dir / "candidate_b" / "transcript.txt"
    jd_path = base_dir / "job_description.txt"

    print("\n[Step 1] Loading Candidate B files & Job Description...")
    ...
    profile.candidate_name = "Ananya Iyer"
    ...
    # Verify 40% accuracy claim landing in unverifiable_claim or red_flag_concern
    accuracy_fact = [f for f in profile.facts if "40%" in f.source_quote or "accuracy" in f.fact.lower()]
    print("\n" + "=" * 80)
    print("            40% ACCURACY METRIC FACT VERIFICATION CHECK")
    print("=" * 80)
    if accuracy_fact:
        print(f"[SUCCESS] Captured {len(accuracy_fact)} fact(s) regarding accuracy metric:")
        for f in accuracy_fact:
            print(f"  - Category: {f.category}")
            print(f"  - Fact: {f.fact}")
            print(f"  - Quote: \"{f.source_quote}\"")
    else:
        print("[WARNING] Accuracy metric fact was not explicitly captured.")
    ...
    # 6. Check Hiring Manager JD Line Quotes & Multi-Agent Gap Assessment
    ...
    jd_keywords = ["multi-agent", "python", "react", "freight", "rag", "vector search", "gap", "production"]
    ...
    print("\n[SUCCESS] Candidate B live evaluation complete!")
